...
```sql
SELECT c.crew_id, c.nickname, COUNT(a.attendance_date) AS `기록된 날짜 수`
FROM crew AS c
LEFT JOIN attendance AS a ON c.crew_id = a.crew_id
GROUP BY c.crew_id, c.nickname;
/*
+---------+-----------+----------------------+
| crew_id | nickname  | 기록된 날짜 수       |
+---------+-----------+----------------------+
|       1 | 검프      |                    7 |
|       2 | 구구      |                    6 |
|       3 | 네오      |                    5 |
...
'크루별로'이므로 `WHERE a.start_time IS NOT NULL`로 필터링하면 '아론'이 아예 결과에서 제외된다. 0으로 나타나는 것이 문제의 의도.

```sql
SELECT c.crew_id, c.nickname, COUNT(a.start_time) AS `등교 기록이 있는 날짜 수`
FROM crew AS c
LEFT JOIN attendance as A ON c.crew_id = a.crew_id
GROUP BY c.crew_id, c.nickname;
/*
+---------+-----------+------------------------------------+
| crew_id | nickname  | 등교 기록이 있는 날짜 수           |
+---------+-----------+------------------------------------+
|       1 | 검프      |                                  7 |
|       2 | 구구      |                                  6 |
|       3 | 네오      |                                  5 |
...
```sql
SELECT c.crew_id, c.nickname, MIN(start_time) AS `가장 빠른 등교 시각`, MAX(start_time) AS `가장 늦은 등교 시각`
FROM crew AS c
LEFT JOIN attendance AS a ON c.crew_id = a.crew_id
GROUP BY c.crew_id, c.nickname;
/*
+---------+-----------+-----------------------------+-----------------------------+
| crew_id | nickname  | 가장 빠른 등교 시각         | 가장 늦은 등교 시각         |
+---------+-----------+-----------------------------+-----------------------------+
|       1 | 검프      | 09:45:00                    | 12:55:00                    |
|       2 | 구구      | 09:58:00                    | 10:10:00                    |
|       3 | 네오      | 09:55:00                    | 13:05:00                    |
|      11 | 디노      | 09:55:00                    | 12:57:00                    |
|      10 | 류시      | 09:45:00                    | 13:03:00                    |
|       8 | 리사      | 09:55:00                    | 13:02:00                    |
|       4 | 브라운    | 09:59:00                    | 13:00:00                    |
|       5 | 브리      | 09:55:00                    | 10:20:00                    |
|      12 | 시지프    | 09:52:00                    | 12:58:00                    |
|      15 | 아론      | NULL                        | NULL                        |
|      13 | 어셔      | 09:31:00                    | 09:31:00                    |
|       7 | 워니      | 09:50:00                    | 12:59:00                    |
|       9 | 제임스    | 09:55:00                    | 12:59:00                    |
|      14 | 주니      | 10:00:00                    | 10:00:00                    |
|       6 | 포비      | 09:52:00                    | 13:10:00                    |
+---------+-----------+-----------------------------+-----------------------------+
 */
```
...
- 기본적으로 서브쿼리라는게 있고.
  ...
  - 메인쿼리에서 서브쿼리 참조는 -> 불가능
  - 서브쿼리를 괄호로 묶어서 AS 별칭 지정하면 -> 메인쿼리에서도 참조 가능. (WHERE 절 서브쿼리에는 불가, 반환된 값을 조건식에서 바로 소모하고 끝)
...
- `GROUP BY`를 사용할 때 `SELECT`에 명시한 컬럼 중 집계 함수에 사용하지 않은 컬럼은 모두 `GROUP BY` 절에도 명시한다.
  - Functional dependency를 인식해서 알아서 매칭해주는 경우도 있지만, 표준상으로 명시 필요.
- `COUNT`는 알아서 `NULL`을 빼고 집계한다. 즉, `IS NOT NULL`로 걸러 낼 필요가 없다.
